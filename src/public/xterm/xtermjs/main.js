...
function createTerminal() {
	...
	params: params,
	url: url + '/termproxy',
	success: async function(result) { // Leko
	    var port = encodeURIComponent(result.data.port);
	    ticket = result.data.ticket;

		const token = (await (await fetch("/token", {method: 'POST', credentials: 'include'})).json());
	    socketURL = protocol + location.hostname + ((location.port) ? (':' + location.port) : '') + '/api2/json' + url + '/vncwebsocket?port=' + port + '&vncticket=' + encodeURIComponent(ticket) + '&token=' + encodeURIComponent(token);
